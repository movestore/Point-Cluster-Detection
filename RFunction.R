...
tz_fn <- function(x) {
  ts <- x[1]
  tz <- str_split_1(x[2], ";")[1]
  output <- as.character(lubridate::with_tz(ts, tz))
  return(output)
}
...
rFunction <- function(meth = "buff", rad = NULL, dur = NULL, minloc = NULL, dur_unit = "days", maxgap = 1, gap_unit = "days", clu_transm = "all", new_dur = 24, data, ...) {
  ...
  dur_udunits <- if (dur_unit != "mins") dur_unit else "min"
  gap_udunits <- if (gap_unit != "mins") gap_unit else "min"
  ...
  meth_buff <- function(data, rad) {
    ...
    if (len_pts > stp) {
      ...
      data_eq_extr[1:len_pts, ] <- extract(vect(data_eq_buffer_unic), pts_eq)
    } # halleluja :)
  ...
  if (length(cluID) > 0) # include here to calc. number of locations/bursts not in cluster - see below
    {
      ...
      if (!is.null(remo_file_name)) {
        ...
        if ("latitude" %in% names(remo) & "longitude" %in% names(remo)) {
          ...
          if (any(remo_dist < rad)) {
            ...
            logger.info("None of your provided remove locations were close to a cluster.")
          }
        } else {
          logger.info("Your csv file does not include the required columns longitude and latitude. Your required remove locations cannot be taken into account.")
        }
      }
      ...
      result$timestamp.local <- apply(data.frame(mt_time(result), tz_info_result), 1, tz_fn)
      ...
      # mt_as_event_attribute_AK <- function(x,nm) -> this function not necessary, as already mt_track_data dont fit!
      # {
      #  utid <- as.data.frame(mt_track_data(result)[,mt_track_id_column(x)])[,1]
      #  nm_events <- character(dim(x)[1])
      #  for (i in seq(along=utid))
      #  {
      #    ix <- which(mt_track_id(result)==utid[i])
      #    nm_events[ix] <- as.character(as.data.frame(mt_track_data(result))[i,nm])
      #  }
      #  result[,nm] <- nm_events
      #  return(result)
      # }
        ...
        if ("trackid" %in% names(fixr) & "fixrate" %in% names(fixr)) {
          ...
          for (j in seq(along = fixr$trackid))
          {
            if (fixr$trackid[j] %in% fixrates_4result$trackid) {
              ...
              logger.info(paste("trackid", fixr$trackid[j], "is not available in the data set. check your spelling."))
            }
          }
          result <- result |>
          ...
          logger.info("Your csv file does not include the required columns trackid and fixrate. These cannot be added to your results object.")
        }
      }
      ...
      result$timelag <- mt_time_lags(result, units = dur_udunits)
      ...
      timestamp.start.local <- apply(data.frame(timestamp.start, tz_info_clu), 1, tz_fn)
      timestamp.end.local <- apply(data.frame(timestamp.end, tz_info_clu), 1, tz_fn)
